...
def get_detections(img_batch, model,score_threshold=0.5, iou_threshold=0.5):
    ...
    with torch.no_grad():
        ...
        for i in range(batch_size):
            ...
            keep_boxes = keep_boxes.cpu().numpy().tolist()
            keep_boxes = np.squeeze(keep_boxes).tolist()
            
            keep_scores = keep_scores.cpu().numpy().tolist()
            keep_scores = np.squeeze(keep_scores).tolist()
            keep_landmarks = keep_landmarks.cpu().numpy().tolist()
            keep_landmarks = np.squeeze(keep_landmarks).tolist()
            # print(keep_boxes.cpu().numpy().tolist())
